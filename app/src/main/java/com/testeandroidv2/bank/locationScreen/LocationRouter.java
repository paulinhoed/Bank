...
interface LocationRouterInput {
    public Intent navigateToSomeWhere(int position);

    public void passDataToNextScene(int position, Intent intent);
}

public class LocationRouter implements LocationRouterInput, AdapterView.OnItemClickListener {

    public static String TAG = LocationRouter.class.getSimpleName();
    public WeakReference<LocationActivity> activity;
    ...
    public void passDataToNextScene(int position, Intent intent) {
        ...
        // LocationModel flight = activity.get().listOfSomething.get(position);
